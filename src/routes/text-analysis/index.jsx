...
export default component$(() => {
  // Check if we have file data from FanFictionDashboard
  const fileContent = useSignal('');
  const fileName = useSignal('');
  const charactersFromStorage = useSignal([]);

  useVisibleTask$(() => {
    // Load file content from sessionStorage if available
    const storedContent = sessionStorage.getItem('ficflow_file_content');
    const storedName = sessionStorage.getItem('ficflow_file_name');
    const storedCharacters = sessionStorage.getItem('ficflow_characters');
    
    if (storedContent) {
      fileContent.value = storedContent;
      sessionStorage.removeItem('ficflow_file_content');
    }
    if (storedName) {
      fileName.value = storedName;
      sessionStorage.removeItem('ficflow_file_name');
    }
    if (storedCharacters) {
      try {
        const chars = JSON.parse(storedCharacters);
        charactersFromStorage.value = chars;
        sessionStorage.removeItem('ficflow_characters');
      } catch (e) {
  ...
  // Use file content if available, otherwise use mock data
  const initialContent = useComputed$(() => {
    return fileContent.value || MOCK_TEXT;
  });

  // Keywords with their assigned colors - use characters from storage if available
  const keywords = useComputed$(() => {
    if (charactersFromStorage.value.length > 0) {
      return charactersFromStorage.value.map((char, index) => ({
        word: char.name,
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
          ...
          {fileName.value && (
            <div class="p-6 bg-white border-b border-gray-200">
              <a 
                href="/fanfiction"
                class="text-sm text-purple-600 hover:text-purple-700 font-medium transition-colors mb-2 inline-block"
              >
                ← Back to Fan Fiction Dashboard
              </a>
              <h2 class="text-2xl font-bold text-gray-900 mb-2">{fileName.value}</h2>
            </div>
          )}
        ...
        <div class="w-80 bg-white border-l border-gray-200 flex flex-col overflow-hidden">
          ...
          <div class="p-6 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">
              {charactersFromStorage.value.length > 0 ? 'Character Legend' : 'Keyword Legend'}
            </h3>
            <div class="space-y-2">
              {keywords.value.map((keyword, index) => {
                const count = stats.value.highlightedCounts[index]?.count || 0;
                return (
                  <div 
                    key={index}
